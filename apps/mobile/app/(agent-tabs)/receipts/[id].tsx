...
export default function AgentReceiptDetail() {
  const { id } = useLocalSearchParams<{ id: string }>();
  const query = useReceipt(id);
  const user = useAuthStore((s) => s.user);
  const bottomInset = useBottomInset(spacing.xxl + spacing.xs);
  // Pemulung berada di sisi penerbit pada bukti yang diterbitkannya sendiri.
  const issuerName =
    user && query.data && query.data.issuedById === user.id ? user.name : undefined;
      ...
      <ScrollView contentContainerStyle={[s.content, { paddingBottom: bottomInset }]}>
        {query.isLoading ? (
          ...
          <ErrorState
            ...
            testID="agent-receipt-error"
          />
        ) : (
          <ReceiptView receipt={query.data} issuerName={issuerName} />
        )}
      </ScrollView>
    </SafeAreaView>
  );
